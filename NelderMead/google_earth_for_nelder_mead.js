...
var GoogleEarthForNelderMead = (function (_super) {
    ...
    GoogleEarthForNelderMead.getElevationForSimplex = function (simplex) {
        ...
        simplex.vertexes.forEach(function (coordinate) {
            locations.push(new google.maps.LatLng(coordinate.latitude, coordinate.longitude));
        });
        if (simplex.reflectionPoint !== null && simplex.expandPoint !== null && simplex.insideContractPoint !== null && simplex.outsideContractPoint !== null && simplex.shrinkPoints !== []) {
            locations.push(new google.maps.LatLng(simplex.reflectionPoint.latitude, simplex.reflectionPoint.longitude));
            locations.push(new google.maps.LatLng(simplex.expandPoint.latitude, simplex.expandPoint.longitude));
            locations.push(new google.maps.LatLng(simplex.insideContractPoint.latitude, simplex.insideContractPoint.longitude));
            locations.push(new google.maps.LatLng(simplex.outsideContractPoint.latitude, simplex.outsideContractPoint.longitude));
            for (var i = 0; i < simplex.shrinkPoints.length; i++) {
                locations.push(new google.maps.LatLng(simplex.shrinkPoints[i].latitude, simplex.shrinkPoints[i].longitude));
            }
        }
     ...
     */
    GoogleEarthForNelderMead.prototype.drawSimplex = function (vertexes) {
        // パン
        var lookAt = this.ge.createLookAt('');
        lookAt.setLatitude(vertexes[0].latitude);
        lookAt.setLongitude(vertexes[0].longitude);
        // ズーム
        var normA = Math.sqrt(Math.pow(vertexes[2].latitude - vertexes[0].latitude, 2) + Math.pow(vertexes[2].longitude - vertexes[0].longitude, 2));
        var normB = Math.sqrt(Math.pow(vertexes[1].latitude - vertexes[0].latitude, 2) + Math.pow(vertexes[1].longitude - vertexes[0].longitude, 2));
