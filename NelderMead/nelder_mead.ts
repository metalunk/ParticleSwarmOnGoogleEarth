...
module NelderMead {
    ...
    export class Simplex {
        ...
        shrinkPoints:Coordinate.Coordinate[];
        ...
        listUpCandidate() {
            ...
            this.shrinkPoints = [];
            for (var i = 0; i < N - 1; i++) {
                this.shrinkPoints[i] = new Coordinate.Coordinate(
                    (this.vertexes[0].longitude + this.vertexes[i + 1].longitude) * EPSILON,
                    (this.vertexes[0].latitude + this.vertexes[i + 1].latitude) * EPSILON
                );
            }
        }
         ...
         */
        sortVertex() {
            this.vertexes.sort(function (a:Coordinate.Coordinate, b:Coordinate.Coordinate) {
        ...
        updateSimplex() {
            ...
            if (
                this.reflectionPoint === null || this.expandPoint === null ||
                this.outsideContractPoint === null || this.insideContractPoint === null
            ) {
                // 初回は候補点がないため変形をしない
                return;
            }
            ...
            if (this.reflectionPoint.getElevation() > this.vertexes[0].getElevation()) {
                ...
                this.sortVertex();
                return;
            } else if (this.reflectionPoint.getElevation() > this.vertexes[N - 1].getElevation() &&
                this.reflectionPoint.getElevation() <= this.vertexes[N - 2].getElevation()
            ) {
        ...
        calcSpace() {
            ...
            return Math.sqrt(
                    (Math.pow(vectorA[0], 2) + Math.pow(vectorA[1], 2) + Math.pow(vectorA[2], 2)) *
                    (Math.pow(vectorB[0], 2) + Math.pow(vectorB[1], 2) + Math.pow(vectorB[2], 2)) -
                    Math.pow(vectorA[0] * vectorB[0] + vectorA[1] * vectorB[1] + vectorA[2] * vectorB[2], 2)
                ) / 2;
        }
     ...
     * 終了時に実行
     */
    function finish(simplex:Simplex) {
        // TODO
        console.log(simplex);
    }
