/// <reference path="google_earth_for_particle_swarm.ts"/>
...
module ParticleSwarm {
    ...
    var ge:GoogleEarthForParticleSwarm;
     ...
     * 最終結果の出力
     *
     * @param ge {GoogleEarthForParticleSwarm}
     * @param bestCoordinate
     */
    function finish(ge:GoogleEarthForParticleSwarm, bestCoordinate:Coordinate.Coordinate) {
     ...
     * @param bestCoordinate {Coordinate}
     */
    function moveSwarm(ge:GoogleEarthForParticleSwarm, loop:number, particles:Particle[], bestCoordinate:Coordinate.Coordinate) {
        var promise = GoogleEarthForParticleSwarm.getElevationForParticles(particles);

        promise.done(function () {
            // 全体の最良地点を求める
            for (var j = 0; j < particles.length; j++) {
                if (bestCoordinate === null ||
                    bestCoordinate.getElevation() < particles[j].coordinate.getElevation()
                ) {
            ...
            ge.drawArrowForParticles(particles);
    ...
    export function main() {
        ge = new GoogleEarthForParticleSwarm();
